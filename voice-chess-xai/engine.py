# Chess library import kar rahe hain —
# jisse board, squares, moves sab kuch milta hai.
import chess

# Simple evaluation function values
# Key = piece ka type (Pawn, Knight, etc — ye chess library ke andar defined constants hain)
# Value = us piece ki "worth" (kitna important hai)
PIECE_VALUES = {
    chess.PAWN: 10,
    chess.KNIGHT: 30,
    chess.BISHOP: 30,
...
# taaki AI ko pata chale ye position bahut important hai.
    ...
    for sq in chess.SQUARES: #chess.SQUARES — chess library ka ek built-in list hai jisme saare 64 squares hain board ke (a1, a2, ... h8)
        piece = board.piece_at(sq) #board.piece_at(sq) — check karta hai ki us specific square pe koi piece hai ya nahi. Agar hai, to us piece ki info deta hai (kaunsa piece, kiska color). Agar khaali square hai, to None deta hai.
        if piece:
...
#  ye classical AI technique hai (jaise purane chess engines mein use hoti thi).
