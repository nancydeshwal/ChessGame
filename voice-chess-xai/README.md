...
- 🤖 **Trainable Neural Network** — an `MLPRegressor` learns board evaluation from real game data and blends with the heuristic engine
...
| 🤖 **Neural Network Evaluation** | Optional `MLPRegressor` (scikit-learn), trained on real FEN/evaluation data, blended 50/50 with the heuristic score |
...
├── engine.py              # AI engine — Minimax, Alpha-Beta pruning, hybrid evaluation
...
├── nn_eval.py              # Neural network evaluation — feature extraction + cached model loading
├── train_nn.py             # One-time training script for the neural network evaluator
├── chess_nn_model.pkl      # Trained MLPRegressor model (generated by train_nn.py)
├── fen_analysis.csv        # Training data — FEN positions with engine evaluations
...
**`engine.py`** — The AI brain. Implements Minimax with Alpha-Beta pruning for efficient tree search. Uses `evaluate_board_hybrid()` to blend the classic piece-value heuristic with the neural network's prediction (falling back to the pure heuristic if no trained model is available).
...
**`nn_eval.py`** — Converts a board into a 12-number feature vector (piece counts per type/color + center-square occupancy) and feeds it to the cached, trained model to produce a predicted evaluation score. Returns `None` gracefully if no model is trained yet.

**`train_nn.py`** — Loads `fen_analysis.csv`, converts each FEN position into features via `nn_eval.extract_features()`, parses both centipawn and mate-notation (`#N`) evaluations, and trains an `MLPRegressor` (two hidden layers, 32 and 16 neurons) which is saved to `chess_nn_model.pkl`.
...
| Board Evaluation | Heuristic + Neural Network (`scikit-learn` `MLPRegressor`) |
...
# 3. (Optional) Train the neural network evaluator
#    Requires fen_analysis.csv (a Kaggle chess-evaluations dataset) in the project folder.
#    If skipped, the app runs fine using the pure heuristic evaluation.
python train_nn.py
